...
2. 当前主题版本为 `1.1.2`，要求 Halo 版本为 `>=2.22.1`，以 `theme.yaml` 中的实际值为准。
...
## 打包与版本约定
1. 每次执行主题打包前，必须先将 `theme.yaml` 中的 `spec.version`（主题版本）在当前版本基础上增加 `0.0.1`。
2. 版本增加 `0.0.1` 表示补丁版本加一，例如 `1.1.1` 升级为 `1.1.2`，不将三段版本号当作小数计算。
3. 打包文件名必须使用升级后的版本号，默认格式为 `theme-stellar-{version}.zip`。
4. 打包前应校验 YAML、必需主题文件和当次新增资源；打包后应校验压缩包内的版本号、根目录结构和必需文件。
5. 除非用户明确指定其他版本号，不得跳过版本递增直接重新打包。
...
2. `settings.yaml`（主题设置入口）：定义 Halo 后台中的主题配置表单。当前分组包括 logo、侧边栏导航、站点主结构树、样式、评论、友链页、页脚、文章、小工具、图库、默认图片、扩展插件、API 地址和扩展设置。页脚配置中的版权信息、ICP 备案号和公安网备号位于自定义 HTML 代码配置之前。
